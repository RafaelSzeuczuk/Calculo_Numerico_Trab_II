...
public class Secante {
    public static String executar(double x0, double x1, double tolerancia, int maximoIteracoes, DoubleUnaryOperator f) {
        ...
        for (int k = 1; k <= maximoIteracoes; k++) {
            ...
            relatorio.append(String.format("%-5d %-14.8f %-14.8f %-14.8f %-14.8f%n", k, xAnterior, xAtual, novoX, fNovoX));
        ...
        relatorio.append("Última aproximação: x = " + String.format("%.8f", xAtual) + "\n");
        relatorio.append("f(x) = " + String.format("%.8f", fAtual) + "\n\n");
